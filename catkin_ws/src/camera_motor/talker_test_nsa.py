...
def getDistance( pixelValue  ,   pToCmTable): #tested and works
    dist = 0.0
    isLargerThanLastValue = True
    for i in range(11):
        if(pToCmTable[i][0]>=pixelValue): #if it is greater we overshot
            if(i==0):
                return 0.0
            isLargerThanLastValue = False
            dist = pToCmTable[i][1]+0.0 - ((pToCmTable[i][0] - pixelValue +0.0)/(pToCmTable[i][0] - pToCmTable[i-1][0]+0.0))*10.0
            break
    if(isLargerThanLastValue): #we have to use last marker
        return (pToCmTable[10][1]+ 0.0 +(( pixelValue - pToCmTable[10][0] +0.0)/(pToCmTable[10][0] - pToCmTable[10-1][0]+0.0))*10.0)/100.0 # divide by 100 for meters
    #otherwise return dist
    return dist/100.0 #for meters
        ...
        while True:
            ...
            if(had_counter):
                if(start_time_speed!=None): #end the timer #high priority
                    ...
                    #convert the pixel values to the distance values
                    y_max = getDistance(y_max_value, pixelToCentimeterTable)
                    y_min = getDistance(y_min_value, pixelToCentimeterTable)
                    x_min = getDistance(x_min_value, pixelToCentimeterTable)
                    x_max = getDistance(x_max_value, pixelToCentimeterTable)

                    deltay_mins = (abs(y_max-ratmaxy)+abs(y_min-ratminy))/2.0
                    deltax_mins = (abs(x_max-ratmaxx)+abs(x_min-ratminx))/2.0
                if(ratminx == None): #initialize the first frame values
                    ratminx = getDistance(x_min_value, pixelToCentimeterTable)  # x_min_value is a pixel value , convert to distance
                    ratminy = getDistance(y_min_value, pixelToCentimeterTable)
                    ratmaxx = getDistance(x_max_value, pixelToCentimeterTable)
                    ratmaxy = getDistance(y_max_value, pixelToCentimeterTable)  # we need to get vertical markers for the vertical distance
                ...
                if(end_time_speed!=None):
                    ...
                    speed_ball_y = (deltay_mins)/(end_time_speed-start_time_speed)
                    speed_ball_x = (deltax_mins)/(end_time_speed-start_time_speed)
